    ...
    <div class="relative w-full max-w-6xl h-auto md:h-[700px] bg-white/15 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/20 overflow-hidden flex flex-col md:flex-row m-4">
        ...
        <div class="flex-1 bg-gradient-to-br from-purple-600/80 to-purple-900/80 flex flex-col justify-center items-center p-10 md:p-16 relative overflow-hidden">
            ...
            <div class="relative z-10 text-center text-white">
                <i class="fas fa-user-plus text-8xl md:text-9xl mb-8 animate-pulse-slow drop-shadow-[0_0_20px_rgba(255,255,255,0.5)]"></i>
                <h1 class="text-4xl md:text-5xl font-bold mb-4 drop-shadow-lg">Join Us Today!</h1>
                <p class="text-base md:text-lg opacity-90 leading-relaxed">Create your account and start your journey with us.</p>
                <div class="mt-8">
                    <p class="text-sm opacity-80 mb-3">Already have an account?</p>
                    <a href="/login" class="inline-block px-8 py-3 bg-white/20 backdrop-blur-sm border-2 border-white/40 text-white font-semibold rounded-xl hover:bg-white/30 hover:-translate-y-1 transition-all duration-300 shine-effect relative overflow-hidden">
        ...
        <div class="flex-1 p-4 sm:p-6 md:p-8 lg:p-12 flex flex-col justify-center max-h-screen md:max-h-full overflow-y-auto">
            ...
            <div class="mb-8 flex-shrink-0">
                <h2 class="text-3xl md:text-4xl font-bold text-white mb-2">Register</h2>
                <p class="text-white/70 text-sm md:text-base">Create your account to get started</p>
            ...
            <form action="{{ route('register.save') }}" method="POST" class="flex-1 min-h-0 overflow-y-auto pr-2">
                @csrf
                <!-- Name Input -->
                <div class="mb-4 sm:mb-5">
                    <label for="name" class="block text-white text-xs sm:text-sm font-medium mb-2 pl-1">Full Name</label>
                    <div class="relative">
                        <i class="fas fa-user absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 text-white/60 text-sm"></i>
                        <input type="text" id="name" name="user_name" value="" class="input-focus w-full py-3 sm:py-3.5 px-3 sm:px-4 pl-10 sm:pl-12 border-2 border-white/20 bg-white/10 rounded-xl text-white placeholder-white/50 transition-all duration-300 focus:outline-none focus:border-white/50 focus:bg-white/15 text-sm sm:text-base" placeholder="Enter your full name" required>
                        <span class="inline-flex items-center rounded px-3 py-1.5 text-base text-white"> @error('user_name'){{ $message }}@enderror</span>
                    </div>
                </div>
                ...
                <div class="mb-4 sm:mb-5">
                    ...
                    <div class="relative">
                        ...
                        <input type="password" id="password" name="user_password" class="input-focus w-full py-3 sm:py-3.5 px-3 sm:px-4 pl-10 sm:pl-12 pr-10 sm:pr-12 border-2 border-white/20 bg-white/10 rounded-xl text-white placeholder-white/50 transition-all duration-300 focus:outline-none focus:border-white/50 focus:bg-white/15 text-sm sm:text-base" placeholder="Create a password" required>
                        <span class="inline-flex items-center rounded px-3 py-1.5 text-base text-white"> @error('user_password'){{ $message }}@enderror</span>
                        <i class="fas fa-eye absolute right-3 sm:right-4 top-1/2 -translate-y-1/2 cursor-pointer text-white/60 hover:text-white transition-colors duration-300 text-sm" onclick="togglePassword('password')"></i>
                    </div>
                </div>

                <!-- Submit Button -->
                <button type="submit" class="ripple-effect w-full py-3 sm:py-4 border-none bg-gradient-to-r from-pink-500 to-red-500 text-white text-sm sm:text-base font-semibold rounded-xl cursor-pointer transition-all duration-300 shadow-[0_5px_15px_rgba(245,87,108,0.4)] hover:-translate-y-1 hover:shadow-[0_8px_20px_rgba(245,87,108,0.6)] active:translate-y-0 relative overflow-hidden mb-4">
                    <span class="relative z-10"><i class="fas fa-user-plus mr-2"></i>Create Account</span>
                </button>
